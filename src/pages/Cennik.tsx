...
export default function Cennik() {
  ...
  const filtered = active === 'Wszystkie' ? services : services.filter((s) => s.category === active);
  ...
  return (
    <>
      <SEOHead
        ...
        canonical="https://smileartdental.pl/cennik"
      />
      ...
      <section className="bg-navy pt-20 pb-12">
        <div className="container mx-auto px-4 text-center">
          <p className="text-light text-sm font-body mb-2">
            <Link to="/" className="hover:text-gold transition-colors">Strona główna</Link> / Usługi i cennik
          </p>
          <h1 className="font-heading text-3xl md:text-5xl text-primary-foreground mb-4">Usługi i cennik</h1>
          <p className="text-light text-lg font-body">Sprawdź nasze usługi i ceny — bez ukrytych opłat</p>
        </div>
      </section>

      {/* Filter tabs */}
      <div className="sticky top-16 z-30 bg-background border-b border-border py-3">
        <div className="container mx-auto px-4 flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-body font-semibold transition-all ${
                active === cat
                  ...
                  : 'bg-card text-muted-foreground hover:bg-accent-light'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((s, i) => (
              ...
              >
                <span className="inline-block bg-accent-light text-foreground text-xs font-body font-bold px-2 py-1 rounded-full mb-3">{s.category}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="bg-accent-light text-foreground text-sm font-bold font-body px-3 py-1 rounded-full">{s.price}</span>
          ...
          <div className="mt-16 bg-navy rounded-lg p-8 md:p-12 text-center reveal">
            <h2 className="font-heading text-2xl md:text-3xl text-primary-foreground mb-4">Raty 0%</h2>
            <div className="flex items-center justify-center gap-4 mb-4">
              <span className="bg-accent-light text-primary text-sm font-body font-bold px-4 py-1.5 rounded-full">MediRaty</span>
              <span className="bg-accent-light text-primary text-sm font-body font-bold px-4 py-1.5 rounded-full">Twisto</span>
            </div>
            <p className="text-light font-body max-w-xl mx-auto">
              Rozłóż płatność za leczenie na wygodne raty 0%. Nie musisz czekać — zacznij leczenie już dziś i płać w ratach bez dodatkowych kosztów.
            </p>
          </div>
        </div>
      </main>
